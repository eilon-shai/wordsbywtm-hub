...
- [x] ✅ **[MEDIUM][UX/Frontend] Required fields visual-only asterisk** — `aria-required` on the control (announces "required" without altering the accessible name); asterisk stays decorative. `forked/FormPrimitives.tsx` — *PR-A*
...
- [x] ✅ **[LOW] venture-core (PR-C):** `setCollectionGenerated` conditional/atomic write shipped in **venture-core 1.24.1** (venture-monorepo #321, merged + published). Signed-invite HMAC tests already existed (above).
...
- [ ] 🔜 **4-step "how it works" consistency** — the hub root showed 4 steps while the product landings showed 3 (drift; product pages folded "Review" into the invite step). Aligning to **4 everywhere**, surfacing "Review what came in" as its own step. Needs venture-core's step-count-aware landing grid (**venture-monorepo #323**, open) so 4 steps render a balanced 2×2 / 4-across instead of 3+1. The hub 4-step landing copy is staged on `fix/four-steps-consistency` and lands **with** the repin to that release.
