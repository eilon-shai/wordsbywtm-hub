...
| F-6 | decision | ✅ IMPLEMENTED | Result page | No "Delete this collection" on the post-payment result view. | **Built** (founder reversed the skip): result page now has a delete-collection+tribute control (`confirmPaidDeletion`), **generated-only** — paid-but-ungenerated stays server-forbidden (409 `PAID_NOT_GENERATED`); copy notes auto-deletion at the deadline. `src/app/api/collection/delete/route.ts` + `ResultFlow.tsx`. |
